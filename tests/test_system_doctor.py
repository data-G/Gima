...
class SystemDoctorTests(unittest.TestCase):
    ...
    def test_doctor_report_has_checks_and_plan(self):
        ...
        self.assertIn("master_ai_director_plan", report)
        self.assertIn("autonomy_boundaries", report)
        self.assertIn("criticism_defense_matrix", report)
        ...
        self.assertEqual(report["master_ai_director_plan"]["kind"], "gima_master_ai_director_plan")
        self.assertIn("controller-class machine", report["master_ai_director_plan"]["hardware_reality"])
        self.assertTrue(any(row["task"] == "Any-file learning" for row in report["master_ai_director_plan"]["routing_rules"]))
        self.assertTrue(any(row["agent"] == "Local Executive Controller" for row in report["master_ai_director_plan"]["agent_roles"]))
        ...
        self.assertTrue(any(row["criticism"] == "Self-improvement can regress" for row in report["criticism_defense_matrix"]))
        self.assertTrue(any(row["name"] == "artifact_qa" for row in report["ai_era_requirements"]["requirements"]))
